...
class Compiler:
    ...
    def compile(context: ProjectContext, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        has_financial = (
            "budget" in context.constraints or "cost" in context.goal.lower()
        )
        has_legal = any(
            reg in str(context.constraints).lower()
            for reg in ["gdpr", "hipaa", "pci", "legal"]
        )
        has_security = bool(context.security) or "security" in context.goal.lower()
        has_health = (
            "health" in context.goal.lower() or "medical" in context.goal.lower()
        )
        has_pii = (
            "pii" in str(context.data).lower()
            or "personal" in str(context.data).lower()
        )
    ...
    def stress(context: ProjectContext) -> Dict[str, Any]:
        ...
        if context.proof_budget and context.claim_ledger.claims:
            for claim in context.claim_ledger.claims.values():
                if (
                    claim.origin_tag == OriginTag.UNKNOWN
                    and context.risk_class == RiskClass.R2
                ):
        ...
        if not context.constraints.get("budget"):
            tests["cost_ops_evaluation"]["issues"].append(
...
class Selector:
    ...
    def select(context: ProjectContext) -> Dict[str, Any]:
        ...
        sorted_options = sorted(
            context.options, key=lambda x: x.get("total_score", 0), reverse=True
        )
...
class Committer:
    ...
    @staticmethod
    def commit(
        context: ProjectContext, validation_results: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
