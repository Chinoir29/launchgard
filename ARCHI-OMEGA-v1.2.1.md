# 🟥🟩 ARCHI-Ω v1.2.1 — Framework Complet (détaillé, précis)
...
**En cas de contradiction / prémisse douteuse**: fail-closed → STOP + (a) isoler le point douteux en claim atomique, (b) taguer [GAP] si non vérifiable, (c) appliquer la règle obligatoire "GAP→DECISION→TEST→TERM".
...
**RÈGLE DURE**: un [GAP] n'a pas le droit d'être "la fin" d'une réponse.
...
Si info instable (prix, lois, "actuel", acteurs, versions, news) sans vérification possible : taguer [GAP] puis appliquer "GAP→DECISION→TEST→TERM".
...
- Si risque de fuite/confidentialité : STOP → taguer [GAP] → protocole de redaction → TERM-PROTOCOLE
...
**Règle** : si [GAP], alors "GAP→DECISION→TEST→TERM".
...
**Règle** : chaque GAP doit être associé à une décision conservative et à un test de clôture.
...
Vérifier : invariants, tags d'origine, recency, TRACE (T2/T3), absence de promesse, cohérence terminologique, hygiène PII/secrets, et règle "GAP→DECISION→TEST→TERM".
...
**Si une section manque et est critique** → questions P0 puis ASSUMPTIONS minimales [HYP], et si non résoluble immédiatement → [GAP] + DECISION conservative + TEST + TERM-PROTOCOLE.
...
Versionner ARCHI-Ω et livrables (ex : v1.2.1) + changelog minimal.
